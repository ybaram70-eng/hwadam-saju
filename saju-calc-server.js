(()=>{
  ...
  function one(x){if(!x)return'-';if(typeof x==='string')return x;return`${x.stem??'-'} / ${x.branch??'-'}`}
  ...
  async function calculate(ev){
    ...
    try{
      ...
      const y=Number($('year')?.value),m=Number($('month')?.value),day=Number($('day')?.value),h=Number($('hour')?.value),min=Number($('minute')?.value);
      ...
      try{const b=parent.document.querySelector('.navItem[data-target="analysis"]');if(b)b.click()}catch{}
      setTimeout(()=>{$('result')?.scrollIntoView?.({behavior:'smooth',block:'start'})},100);
    }catch(e){if(st){st.className='status err';st.textContent='계산 오류: '+(e?.name==='AbortError'?'서버 응답이 늦습니다. 다시 눌러 주세요.':(e?.message||String(e)))}}
