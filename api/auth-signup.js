...
export default async function handler(req,res){
  ...
  try{
    ...
    const {name,password,oldPassword,newPassword,phone,birthDate,gender,calendarType,agree}=req.body||{};
    ...
    if(nextPassword.length<8)return res.status(400).json({ok:false,error:'PASSWORD_TOO_SHORT'});
    ...
    if(exists.length){
      const old=exists[0];
      const verifyWith=String(oldPassword||firstPassword||'');
      if(!verifyPassword(verifyWith,old.password_salt,old.password_hash)){
      ...
      return res.status(200).json({ok:true,user:u,reRegistered:true,passwordChanged:true});
    }
