(() => {
  if (window.__hwadamConsultationProductsBooted) return;
  window.__hwadamConsultationProductsBooted = true;
  const d = document;
  ...
  function clearAiWatch(){if(aiWatch){clearInterval(aiWatch);aiWatch=null}}
  function startAiWatch(){
    clearAiWatch();
    const p=readSelected();
    if(!p?.id||p.id==='annual-membership')return;
    const before=readLast();
    const beforeSig=(before.reportId||'')+'|'+(before.answer||'');
    let tries=0;
    aiWatch=setInterval(()=>{
      const now=readLast();
      const sig=(now.reportId||'')+'|'+(now.answer||'');
      if(now.answer&&sig!==beforeSig){
        clearAiWatch();
        d.dispatchEvent(new CustomEvent('hwadam:consult-complete',{detail:{report:now,product:readSelected()}}));
        d.dispatchEvent(new CustomEvent('hwadam:product-selected',{detail:{...readSelected(),refresh:true}}));
        return;
      }
      if(++tries>90)clearAiWatch();
    },500);
  }
  function annualYear(){try{return Number(localStorage.getItem('hwadam_annual_year'))||THIS_YEAR}catch{return THIS_YEAR}}
  function annualVariant(p,year=annualYear()){if(p.id!=='annual-fortune')return p;const y=ANNUAL_YEARS.includes(Number(year))?Number(year):THIS_YEAR;return {...p,name:`${y}년 신년운세`,q:`${y}년 한 해의 전체 흐름과 1월부터 12월까지의 월별 운세를 재물, 직업과 사업, 관계, 건강 중심으로 자세히 설명해 주세요.`}}
  function renderAnnualYears(){
    const sec=$('hwadamProductCatalog');if(!sec)return;
    let box=$('hwadamAnnualYears');
    if(!box){box=d.createElement('div');box.id='hwadamAnnualYears';box.className='hpcYears';const grid=sec.querySelector('.hpcGrid');grid?.insertAdjacentElement('afterend',box)}
    const y=annualYear();
    box.innerHTML=`<b>보고 싶은 연도를 선택하세요</b><div>${ANNUAL_YEARS.map(v=>`<button type="button" data-year="${v}" class="${v===y?'on':''}">${v}년</button>`).join('')}</div>`;
    box.querySelectorAll('button').forEach(btn=>btn.onclick=()=>{try{localStorage.setItem('hwadam_annual_year',btn.dataset.year)}catch{}selectProduct(PRODUCTS.find(x=>x.id==='annual-fortune'),Number(btn.dataset.year));renderAnnualYears()});
    box.hidden=false;
  }
  function selectProduct(p,year){
    ...
    try{
      ...
      if(p.id!=='annual-membership')localStorage.removeItem('hwadam_last_ai_consult');
    }catch{}
  ...
  function boot(){
    let tries=0;const tryBuild=()=>{if(build())return;if(++tries<25)setTimeout(tryBuild,200)};tryBuild();
    d.addEventListener('click',e=>{if(e.target.closest('#aiAsk'))setTimeout(startAiWatch,80)},true);
  }
  window.hwadamConsultationProducts={PRODUCTS,selectProduct,startAiWatch};
