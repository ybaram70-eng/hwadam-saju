...
export async function ensureAuthTables(){const s=db();if(!s)return false;await s`create table if not exists hwadam_users (id bigserial primary key, name text not null, email text unique, phone text, birth_date date, gender text, calendar_type text not null default 'solar', password_salt text not null, password_hash text not null, created_at timestamptz not null default now())`;await s`alter table hwadam_users alter column email drop not null`;await s`alter table hwadam_users add column if not exists calendar_type text not null default 'solar'`;await s`create unique index if not exists hwadam_users_phone_uidx on hwadam_users(phone) where phone is not null`;await s`create table if not exists hwadam_sessions (token_hash text primary key, user_id bigint not null references hwadam_users(id) on delete cascade, expires_at timestamptz not null, created_at timestamptz not null default now())`;return true}
